...
4. Traverse AST with `SqlVisitor` to build `sqlglot` expression tree.
   ...
   - The portable date functions become sqlglot's typed nodes, so each dialect renders its own spelling: `YEAR(x)`, `MONTH(x)`, `DAY(x)`, `EXTRACT('year'|'month'|'day', x)` and `DATE_TRUNC('year'|'month'|'day', x)` render as `EXTRACT(YEAR FROM x)` and `DATE_TRUNC('YEAR', x)` on Postgres, `DATETRUNC(YEAR, x)` on T-SQL, and so on. SQLite has none of these and sqlglot does not translate them, so on SQLite they are rewritten with the same meaning: a date part is `CAST(STRFTIME('%Y', x) AS INTEGER)` (`%m`, `%d`), and a truncation is `DATE(x, 'start of year')`, `DATE(x, 'start of month')` or `DATE(x)`. Any other function is rendered as written.

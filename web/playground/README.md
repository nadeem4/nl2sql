...
ledger, refused-table parsing, the trace drill-down helpers) and `src/settings.js`
(model options, which nodes changed, which chosen models run without a
temperature) with Node's built-in test runner; there is no test dependency.
...
Both are SIL Open Font License. The built page is about 291 KB.
...
and the settings routes (`GET /api/settings`, `POST /api/settings/key`,
`POST /api/settings/models`). The settings routes refuse a request whose
`Origin` is not the page's own, so change settings from the page `nl2sql demo`

...
PLANNER_PROMPT = (
    ...
    "[DATATYPE RULES]\n"
    "- **Date/Time**: The datasource expects dates in this format: '{date_format}'.\n"
    "- **Literals**: Use single quotes for strings/dates. Do NOT quote numbers unless they are string columns.\n"
    "- **Type Awareness**: The [SCHEMA] provides column types (e.g., INTEGER, VARCHAR). Use this to determine if quotes are needed.\n\n"
    "[CONSTRAINTS]\n"
    ...
    "- Columns in [SCHEMA] have type info (e.g. `t1.id` (INTEGER)). Use ONLY the name part `t1.id` in `expr`.\n\n"
    "[SCHEMA]\n"
    "{schema_context}\n"
    "[INPUT]\n"
    "{intent_context}"

...
class FakeAdapter:
    ...
    def __call__(self, **kwargs):
        # The registry builds adapters from a class; this stands in for one.
        return self
...
def _ctx(adapters, monkeypatch):
    # A real registry, so the node goes through its one capability check.
    ...
    for ds_id in adapters:
        registry.register_datasource(DatasourceConfig(id=ds_id, connection=ConnectionConfig(type=ds_id)))
    return SimpleNamespace(ds_registry=registry, tenant_id="t1")
...
def test_executor_node_runs_sql_executor_for_sql_capable_datasource(monkeypatch):
    ...
    node = ExecutorNode(_ctx({"sql_ds": FakeAdapter({DatasourceCapability.SUPPORTS_SQL})}, monkeypatch))
...
def test_executor_node_refuses_datasource_without_sql_capability(monkeypatch):
    ...
    # Arrange
    _patch_execute(monkeypatch)
    node = ExecutorNode(_ctx({"rest_ds": FakeAdapter({DatasourceCapability.SUPPORTS_REST})}, monkeypatch))
...
def test_executor_node_refuses_datasource_when_capability_lookup_raises(monkeypatch):
    ...
    # Arrange
    _patch_execute(monkeypatch)
    node = ExecutorNode(_ctx({"broken_ds": FakeAdapter(RuntimeError("boom"))}, monkeypatch))

...
from nl2sql.common.errors import ErrorSeverity, ErrorCode
...
def demo_env() -> SimpleNamespace:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="physical_validator_real_data_"))
    secrets_path = _write_empty_secrets(tmp_dir)

    monkeypatch = pytest.MonkeyPatch()
    collection_name = f"itest_physical_validator_{uuid.uuid4().hex}"
...
def test_physical_validator_real_data(demo_env) -> None:
    datasources = _load_demo_datasources(demo_env.root)
    validator = PhysicalValidatorNode(demo_env.ctx, row_limit=100000)
    ...
    for ds in datasources:
        ...
        errors = result["physical_validator_response"].errors
        assert all(
            e.severity not in (ErrorSeverity.CRITICAL, ErrorSeverity.ERROR)
            or e.error_code == ErrorCode.SERVICE_UNAVAILABLE
            for e in errors
        )
